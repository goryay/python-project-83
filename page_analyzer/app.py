...
def post_url():
    ...
    if messages:
        for message in messages:
            flash(*message)
        return render_template('index.html', url=url_entered), 422
    ...
    if url:
        flash('Старница уже существует', 'info')
    else:
...
def show_url(url_id):
    ...
    url_checks = db.get_checks(conniction, url_id)
    db.close(connection)
